...
router.post('/register',
  ...
  async (req, res, next) => {
    try {
      ...
      // Create user — auto verify if email not required
      ...
      // Send verification email only if SendGrid is configured
      if (
        requireVerify &&
...
router.post('/login',
  ...
  async (req, res, next) => {
    try {
      ...
      // Only block unverified users if REQUIRE_EMAIL_VERIFY is true
...
router.post('/refresh', async (req, res, next) => {
  try {
    ...
    const valid = user.refreshTokenHash &&
...
router.get('/verify-email/:token', async (req, res, next) => {
  try {
    ...
    res.json({
      success: true,
      message: 'Email verified! You can now log in to 50/50 Life.'
    });
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          logger.warn('Password reset email failed: ' + emailErr.message);
        }
      }
